...
class Database:
    ...
    async def _get_connection(self) -> aiosqlite.Connection:
        ...
        При первом подключении автоматически инициализирует БД (создаёт таблицы и выполняет миграции),
        если она ещё не была инициализирована.
    ...
    async def _init_database(self):
        """
        Инициализирует БД: создаёт все таблицы, если их нет, и выполняет миграции.
        
        Функция выполняет следующие операции:
        1. Создание таблиц (users, alerts, user_alerts, error_logs), если их нет
        2. Миграции: добавление новых полей (например, password_hash, normalized_symbol)
        3. Удаление устаревших таблиц (например, registration_whitelist)
        4. Автоматическая миграция данных (например, нормализация символов для существующих записей)
        
        Миграции выполняются безопасно: если поле уже существует или таблица уже удалена,
        ошибки игнорируются.
        """
        ...
        try:
            ...
            # Миграция: удаление настроек для устаревших котируемых активов Binance Spot (AUD, GBP, BIDR, AEUR)
            try:
                logger.info("Начинаем миграцию: удаление настроек для устаревших котируемых активов (AUD, GBP, BIDR, AEUR)")
                
                # Список удаляемых активов
                deprecated_quote_assets = ["AUD", "GBP", "BIDR", "AEUR"]
                ...
                async with conn.execute("SELECT id, user, options_json FROM users") as cursor:
                    ...
                    if users:
                        ...
                        for user_row in users:
                            user_id = user_row[0]
                            username = user_row[1]
                            raw_options = user_row[2] if len(user_row) > 2 else '{}'
                            
                            try:
                                # Парсим options_json
                                if not raw_options or raw_options.strip() == '':
                                    raw_options = '{}'
                                ...
                                original_options_str = json.dumps(options, ensure_ascii=False, sort_keys=True)
                                
                                # Удаляем ключи из pairSettings
                                if "pairSettings" in options and isinstance(options["pairSettings"], dict):
                                    for deprecated_asset in deprecated_quote_assets:
                                        key_to_remove = f"binance_spot_{deprecated_asset}"
                                        if key_to_remove in options["pairSettings"]:
                                            del options["pairSettings"][key_to_remove]
                                            options_changed = True
                                            logger.debug(f"Удалён ключ {key_to_remove} из pairSettings пользователя {username}")
                                
                                # Удаляем ключи из chartSettings
                                if "chartSettings" in options and isinstance(options["chartSettings"], dict):
                                    for deprecated_asset in deprecated_quote_assets:
                                        key_to_remove = f"binance_spot_{deprecated_asset}"
                                        if key_to_remove in options["chartSettings"]:
                                            del options["chartSettings"][key_to_remove]
                                            options_changed = True
                                            logger.debug(f"Удалён ключ {key_to_remove} из chartSettings пользователя {username}")
                                
                                # Сохраняем обновленные настройки, если были изменения
                                if options_changed:
                                    updated_options_str = json.dumps(options, ensure_ascii=False, sort_keys=True)
                                    await conn.execute("""
                                        UPDATE users 
                                        SET options_json = ?, updated_at = CURRENT_TIMESTAMP
                                        WHERE id = ?
                                    """, (updated_options_str, user_id))
                                    updated_count += 1
                                    logger.info(f"Обновлены настройки пользователя {username} (ID: {user_id})")
                                
                            except json.JSONDecodeError as e:
                                error_count += 1
                                logger.warning(
                                    f"Ошибка парсинга JSON для пользователя {username} (ID: {user_id}): {e}"
                                )
                            except Exception as e:
                                error_count += 1
                                logger.warning(
                                    f"Ошибка при миграции настроек для пользователя {username} (ID: {user_id}): {e}"
                                )
                        ...
                        logger.info(
                            f"Миграция настроек завершена: обновлено {updated_count} пользователей, "
                            f"ошибок {error_count}"
                        )
                    else:
                        logger.info("Нет пользователей для миграции настроек")
            except Exception as e:
                logger.error(f"Ошибка при миграции настроек для устаревших активов: {e}", exc_info=True)
                # Продолжаем работу, это не критично для работы системы
