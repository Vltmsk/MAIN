...
async def _ws_connection_worker(
    ...
    while True:
        ...
        try:
            ...
            if was_connected:
                _stats[market]["reconnects"] += 1
                logger.info(f"Binance {market} [{connection_id}]: переподключение из-за разрыва (счётчик: {_stats[market]['reconnects']})")
            await on_error({
                "exchange": "binance",
                "market": market,
                "connection_id": connection_id,
            ...
            if was_connected:
                _stats[market]["reconnects"] += 1
                logger.info(f"Binance {market} [{connection_id}]: переподключение из-за ошибки (счётчик: {_stats[market]['reconnects']})")
            await on_error({
                "exchange": "binance",
                "market": market,
                "connection_id": connection_id,
        ...
        try:
            ...
            async with _symbol_update_lock:
                ...
                if removed_symbols or added_symbols:
                    logger.info(
                        f"Binance {market}: обновлен список символов: "
                        f"{len(removed_symbols)} удалено, {len(added_symbols)} добавлено, "
                        f"всего символов: {len(new_symbols)}"
                    )
                else:
                    logger.info(
                        f"Binance {market}: кэш символов обновлен, изменений нет, "
                        f"всего символов: {len(new_symbols)}"
                    )
                ...
                for symbol in removed_symbols:
                    ...
                    for conn_id, (ws, streams_list, conn_market) in _active_connections.items():
                    ...
                    for conn_id, ws, streams_list in connections_to_update:
                        if stream in streams_list:
                            ...
                            if market == "linear" and not ws.closed:
                                try:
                                    unsubscribe_msg = {
                                        "method": "UNSUBSCRIBE",
                                        "params": [stream],
                ...
                for symbol in added_symbols:
                    ...
                    if best_connection and not best_connection.closed:
                        # Добавляем stream в существующее соединение
                        ...
                        if market == "linear":
                            try:
                                ...
                                if stream in best_streams:
                                    best_streams.remove(stream)
                        else:
                        ...
                        new_symbols_to_add = [s for s in added_symbols if _symbol_to_stream(s, market) not in [
                            stream for _, streams_list, m in _active_connections.values() 
                            if m == market for stream in streams_list
                        ]]
