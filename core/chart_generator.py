...
class ChartGenerator:
    ...
    def _get_cache_key(exchange: str, market: str, symbol: str, detection_timestamp: int) -> str:
        ...
        # График показывает 30 минут до детекта, поэтому для каждого уникального момента детекта
        # нужен свой график, чтобы показывать актуальные данные до точного момента детекта
