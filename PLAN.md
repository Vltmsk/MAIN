...
### 1. Исправление ошибок SSL при запросе истории сделок (Gate.io, Bybit, Hyperliquid)
**Проблема:** При запросе истории сделок для генерации графиков возникают ошибки SSL: `SSLCertVerificationError: certificate verify failed: unable to get local issuer certificate` для `api.gateio.ws`, `api.bybit.com`, `api.hyperliquid.xyz`. В результате графики по части инструментов не строятся.

**Задача:**
- Диагностировать причину на уровне окружения (корневые сертификаты Windows, Python, `certifi`, возможный HTTPS‑прокси/антивирус)
- Настроить корректное доверие к корневым сертификатам (обновить `certifi`, при необходимости добавить кастомный CA в используемый `SSLContext`)
- На уровне кода в `core/chart_generator.py` (и обёртки HTTP‑клиента) предусмотреть:
  - Явное использование корректно настроенного `SSLContext`
  - Расширенное логирование причин SSL‑ошибок (версия Python, версия `certifi`, информация о прокси, если есть)
- Обеспечить отсутствие отключения проверки сертификата на проде; если временно используется `verify=False`/`ssl=False` в dev, явно логировать это

**Приоритет:** Высокий

---

### 2. Исправление ошибок HTTP 400 при запросе сделок Bitget
**Проблема:** При запросе истории сделок к Bitget для генерации графиков систематически возвращается `HTTP 400` для ряда инструментов (`TNSRUSDT`, `TRUSTUSDT`, `BEATUSDT`, `ALCHUSDT`, `COAIUSDT`, `FOLKSUSDT`, `FUSDT`, `HANAUSDT`, `PIEVERSEUSDT`, `XCNUSDT` и др.). В результате по этим монетам не строятся графики, а в логах появляется большое количество однотипных предупреждений.

**Задача:**
- В `core/chart_generator.py`:
  - Логировать тело ответа Bitget (`code`, `msg`) и параметры запроса (endpoint, тип рынка, symbol/instId, таймфрейм, лимиты)
  - Проверить правильность формирования параметров для spot и linear (название тикера, разделители, суффиксы контрактов и т.п.)
- Сопоставить внутренние символы с фактическими тикерами Bitget на основе данных из `exchanges/bitget/symbol_fetcher.py` и БД нормализации символов
- При необходимости доработать маппинг/нормализацию символов для Bitget (учёт разных форматов тикеров и рынков)
- Реализовать кэш/чёрный список для проблемных символов: после нескольких `HTTP 400` помечать символ как временно недоступный для графиков и не спамить API повторными запросами
- В интерфейсе/уведомлениях явно сообщать, что по выбранному инструменту Bitget не отдаёт историю сделок, поэтому график недоступен

**Приоритет:** Высокий

---

### 3. Улучшение устойчивости WebSocket‑соединений и подписок (Gate, Hyperliquid)
**Проблема:** В логах регулярно появляются ошибки WebSocket:
- Gate: `Ошибка в WS соединении Gate LINEAR#N: no close frame received or sent`
- Hyperliquid: предупреждения «Соединение закрыто при подписке на SYMBOL»

Это приводит к временному отсутствию потока данных по части символов и засоряет логи.

**Задача:**
- В `exchanges/gate/ws_handler.py` и `exchanges/hyperliquid/ws_handler.py`:
  - Проверить и доработать логику авто‑реконнекта при любых обрывах (включая случаи без корректного close frame)
  - Хранить и восстанавливать список активных подписок при переподключении
  - Реализовать экспоненциальные задержки между попытками переподключения
- Ограничить число символов на одно WS‑соединение и частоту отправки подписок (batched‑подписка с небольшими задержками между пакетами)
- Улучшить логирование:
  - Добавить информацию о символах, прикреплённых к конкретному соединению (Gate LINEAR#0, #2, #5 и т.п.)
  - Логировать серверные коды/сообщения об ошибках, если они приходят
- Пересмотреть уровни логов: ожидаемые/автоматически обрабатываемые переподключения перевести из `ERROR` в `WARNING` или `INFO`, чтобы критические ошибки было проще отличать от нормальной работы

**Приоритет:** Средний

---

### 4. Повышение надёжности отправки уведомлений в Telegram
**Проблема:** В логах регулярно появляются ошибки вида `telegram_error` и `telegram_timeout`: «Не удалось отправить уведомление пользователю ...: Таймаут при подключении к Telegram API (проверьте интернет-соединение)». При временных сетевых проблемах уведомление часто полностью теряется, отправка не повторяется, а ошибки засоряют логи и не разделяются по типам (сетевая ошибка, неверный chat_id, блокировка бота и т.п.).

**Задача:**
- В модуле отправки уведомлений в Telegram (например, `core/telegram_notifier.py` и связанные вызовы в `main.py`):
  - Добавить механизм повторных попыток (ретраев) с экспоненциальной задержкой при сетевых ошибках и таймаутах (например, до 3–4 попыток с увеличением интервала)
  - Настроить разумные таймауты подключения и ответа Telegram Bot API
  - Обеспечить, чтобы временные проблемы с Telegram не блокировали основную логику детектора (отправка выполняется асинхронно или через очередь задач)
- Разделить уровни и типы логирования:
  - Сетевые/временные ошибки и таймауты — отдельный тип (`telegram_timeout` или `WARNING`)
  - Логические ошибки (неверный chat_id, блокировка бота, 403/401) — `telegram_error` с уровнем `ERROR` и понятным текстом
- (Опционально) Добавить учёт статистики доставок:
  - Счётчики успешных/неуспешных отправок по пользователям/чатам
  - Возможность в UI или административном интерфейсе видеть, если по какому-то пользователю долго были проблемы с доставкой уведомлений
...
2. **Множественные графики для одного события**
   ...
   - **Примечание:** Текущее поведение корректно - если несколько условных шаблонов подходят, все они отправляются. Если подходит условный шаблон, обычный не отправляется.

3. **Ошибки при отправке без уведомления пользователя**
   - Если отправка графика не удастся, пользователь не получает уведомление
   - Решение: При ошибке отправки графика отправлять текстовое сообщение с информацией об ошибке
   - ✅ **ИСПРАВЛЕНО**
...
4. **Дублирование генерации графиков**
   ...
   - **Примечание:** Кэш уже работает на уровне генератора графиков, проблема решена через кэширование

5. **Отсутствие дедупликации Chat ID при отправке графиков**
   - Если несколько условных шаблонов указывают на один и тот же Chat ID, график отправляется несколько раз в один чат
   - Решение: Дедуплицировать Chat ID перед отправкой - собирать уникальные Chat ID и отправлять график один раз в каждый уникальный чат с соответствующими подписями
   - **Приоритет:** Средний
...
- [ ] 7. Оптимизация производительности страницы статистики через нормализацию символов при записи в БД
- [ ] 8. Исправление проблем с отправкой графика вместе с текстом
- [ ] 9. Исправление ошибок SSL при запросе истории сделок (Gate.io, Bybit, Hyperliquid)
- [ ] 10. Исправление ошибок HTTP 400 при запросе сделок Bitget
- [ ] 11. Улучшение устойчивости WebSocket‑соединений и подписок (Gate, Hyperliquid)
- [x] 12. Повышение надёжности отправки уведомлений в Telegram
